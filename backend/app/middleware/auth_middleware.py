...
def require_verified_trader(f):
    """Decorator: require valid JWT + pro_trader role + is_verified = True."""
    @wraps(f)
    def decorated(*args, **kwargs):
        try:
            verify_jwt_in_request()
        ...
        if not profile.is_verified:
            return jsonify({
                ...
                "message": "Your account must be verified by an admin before you can perform this action.",
